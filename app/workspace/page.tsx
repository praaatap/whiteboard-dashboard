...
export default function WhiteboardsDashboard() {
  ...
  const handleMoveDashboardToFolder = async (dashboardId: string, folderId: string | null) => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to move dashboard');
  ...
  const filteredDashboards = selectedFolder 
          ...
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative group">
                ...
                >
                  <div className="p-2">
                    <Link href="/profile" className="block px-3 py-2 text-sm text-gray-300 hover:bg-white/5 rounded transition">
                      Profile
                    </Link>
                    <Link href="/settings" className="block px-3 py-2 text-sm text-gray-300 hover:bg-white/5 rounded transition">
                      Settings
                    </Link>
            ...
            >
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  >
                    {Array.isArray(dashboards) ? dashboards.filter(d => d.isPublic).length : 0}
                  </motion.span>
                </motion.div>
              </div>
            </motion.div>
            ...
            >
              ...
              <motion.button
                ...
                className={`w-full flex items-center justify-between p-2 rounded-lg transition text-left mb-2 ${
                  !selectedFolder ? 'bg-indigo-500/20 border border-indigo-500/50' : 'hover:bg-white/5'
                }`}
                whileHover={{ x: 5 }}
                whileTap={{ scale: 0.98 }}
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-300 font-medium">All Boards</span>
                </div>
                <span className="text-xs text-gray-500">{getRootDashboards().length}</span>
              </motion.button>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <motion.svg 
                            ...
                            stroke="currentColor" 
                            viewBox="0 0 24 24"
                            animate={dropTargetFolder === folder.id ? { scale: [1, 1.2, 1] } : {}}
                            transition={{ duration: 0.3 }}
                          >
                        ...
                        <span className="text-xs text-gray-500">{getFolderDashboards(folder.id).length}</span>
                      </motion.button>
                      <motion.button
                        onClick={(e) => {
                          e.stopPropagation();
            ...
            <AnimatePresence>
              {selectedFolder && (
                ...
                >
                  ...
                  >
                    <svg className={`w-4 h-4 text-${selectedFolder.color}-400`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                    </svg>
                    {selectedFolder.name}
                    <motion.button 
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <button className="text-gray-400 hover:text-white transition">Recent</button>
                  <span className="text-gray-600">•</span>
              ...
              {filteredDashboards.length === 0 ? (
                ...
                >
                  ...
                  <h3 className="text-lg font-semibold text-white mb-2">No dashboards found</h3>
                  <p className="text-gray-400 mb-4">
                    {selectedFolder ? "This folder is empty - drag boards here" : "Create your first dashboard to get started"}
                  </p>
                  <motion.button
                    onClick={handleCreateDashboard}
                  ...
                  <AnimatePresence mode="popLayout">
                    {filteredDashboards.map((board, index) => (
                      <motion.div
                        ...
                        onClick={(e) => {
                          if ((e.target as HTMLElement).tagName !== 'BUTTON' && !draggedDashboard) {
                  ...
                  <AnimatePresence mode="popLayout">
                    {filteredDashboards.map((board, index) => (
                      <motion.div
                        ...
                        onDragStart={() => handleDragStart(board)}
                        onDragEnd={handleDragEnd}
                        whileDrag={{ scale: 1.02, boxShadow: "0 20px 25px -5px rgba(99, 102, 241, 0.3)" }}
                        ...
                        whileHover={{ x: 5 }}
                        onClick={() => !draggedDashboard && router.push(`/dashboard/${board.id}`)}
                      >
                        <motion.div 
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-gray-400">
                            {new Date(board.updatedAt).toLocaleDateString()} • {board.viewCount} views
                          </p>
                        </div>
                      </motion.div>
                    ))}
                    ...
                    {colorOptions.map(color => (
                      <motion.button
                        ...
                        className={`w-10 h-10 rounded-lg bg-${color}-500 transition ${
                          newFolderColor === color ? `ring-2 ring-${color}-400 ring-offset-2 ring-offset-[#1a1a1f]` : ''
                        }`}
                        whileHover={{ scale: 1.2 }}
                        whileTap={{ scale: 0.9 }}
                        title={color}
